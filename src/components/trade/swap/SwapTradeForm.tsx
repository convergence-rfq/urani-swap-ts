...
interface SwapTradeFormProps {
  ...
  onGetQuote: (params: { 
    ...
    tokenOut: string; 
    amount: string; 
  }) => Promise<RouterQuote | null>;  // Replace any with specific return type
...
// Update the interface to match Convergence API
  ...
  const onSubmit = useCallback(async () => {
    try {
      if (!wallet.connected || !wallet.signTransaction) {
        setError({ message: "Please connect your wallet", type: 'error' });
        return;
      }

      if (!quoteData) {
        setError({ message: "Please get a quote first", type: 'error' });
        return;
      }
